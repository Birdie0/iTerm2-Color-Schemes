...
def convert(xrdb_colors, termux_out=sys.stdout):
    termux = OrderedDict(XRDB2TERMUX)

    for xrdb_key in termux.keys():
        if xrdb_key in xrdb_colors:
            termux[xrdb_key] = termux[xrdb_key] + xrdb_colors[xrdb_key]
        else:
            termux[xrdb_key] = termux[xrdb_key]
...
def main(xrdb_path, output_path=None):
    for f in glob(join(xrdb_path, '*.xrdb')):
        ...
        termux_out = output_path and join(output_path, base_name + '.properties') or sys.stdout
        convert(xrdb_in, termux_out)


if __name__ == '__main__':

    parser = argparse.ArgumentParser(
        description='Translate X color schemes to Termux .properties format')
    ...
    parser.add_argument('-d', '--out-directory', type=str, dest='output_path',
                        help='path where Termux .properties config files will be' +
